...
### 코드 수정 계층 규칙 (Modification Hierarchy) → [상세: memory/code-modification-convention.md]
...
- **Twin Fix Rule**: CardList ↔ FloatingScratchPad, handleCardDrop ↔ handleScratchPadCardDrop 등 쌍 컴포넌트는 한쪽 수정 시 다른 쪽 반드시 확인
- **참조 안정성**: props로 전달하는 배열/객체는 `useMemo` 래핑 필수, `|| []` 대신 `?? EMPTY_CONST`
- **useEffect filter-only**: 데이터 변경 시 전체 클리어 금지, 존재하지 않는 ID만 필터링
