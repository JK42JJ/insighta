...
const Index = () => {
  ...
  // ✨ 디버깅: 페이지 로드 시 persistedLocalCards 원본 데이터 로깅
  useEffect(() => {
    console.log('[Index] 📦 persistedLocalCards loaded (raw data):', {
      isLoading: isLoadingLocalCards,
      count: persistedLocalCards.length,
      cards: persistedLocalCards.map(c => ({
        id: c.id?.slice(0, 8),
        cellIndex: c.cellIndex,
        levelId: c.levelId,
        url: c.videoUrl?.slice(0, 40),
      })),
    });
  }, [persistedLocalCards, isLoadingLocalCards]);
  ...
  }, [isLoadingPreferences, prefScratchpadFloating, prefScratchpadDock, prefMandalaFloating, prefMandalaMinimized, prefMandalaDock, hasInitializedPreferences]);

  // Handlers that update both local state and persist to preferences
  const handleSetScratchPadFloating = useCallback((floating: boolean) => {
  ...
  const handleSetScratchPadDockPosition = useCallback((position: DockPosition) => {
  ...
  const handleSetMandalaMinimized = useCallback((minimized: boolean) => {
  ...
  const handleSetMandalaFloating = useCallback((floating: boolean) => {
  ...
  const handleSetMandalaDockPosition = useCallback((position: MandalaDockPosition) => {
  ...
  const mandalaLocalCards = useMemo(() => {
    const result = persistedLocalCards.filter(
      c => typeof c.cellIndex === 'number' && c.cellIndex >= 0 && c.levelId && c.levelId !== 'scratchpad'
    );
    // Debug logging
    console.log('[mandalaLocalCards] Filter result:', {
      total: persistedLocalCards.length,
      mandalaCount: result.length,
      cards: result.map(c => ({ id: c.id, cellIndex: c.cellIndex, levelId: c.levelId, url: c.videoUrl?.slice(0, 50) })),
    });
    return result;
  }, [persistedLocalCards]);
  ...
  const scratchpadLocalCards = useMemo(() => {
    ...
    // Debug logging
    console.log('[scratchpadLocalCards] Filter result:', {
      total: persistedLocalCards.length,
      scratchpadCount: result.length,
      cards: result.map(c => ({ id: c.id, cellIndex: c.cellIndex, levelId: c.levelId, url: c.videoUrl?.slice(0, 50) })),
    });
    return result;
  }, [persistedLocalCards]);
  ...
  const mandalaVideoCards = useMemo(() => {
    const result = syncedCards.filter(
      c => c.isInIdeation === false &&
           typeof c.cellIndex === 'number' &&
           c.cellIndex >= 0 &&
           c.levelId &&
           c.levelId !== 'scratchpad'
    );
    console.log('[mandalaVideoCards] Filter result:', {
      total: syncedCards.length,
      mandalaCount: result.length,
      cards: result.map(c => ({ id: c.id, cellIndex: c.cellIndex, levelId: c.levelId, isInIdeation: c.isInIdeation })),
    });
    return result;
  }, [syncedCards]);
  ...
  const ideationVideoCards = useMemo(() => {
    ...
    console.log('[ideationVideoCards] Filter result:', {
      total: syncedCards.length,
      ideationCount: result.length,
      cards: result.map(c => ({ id: c.id, isInIdeation: c.isInIdeation })),
    });
    return result;
  }, [syncedCards]);
  ...
  const scratchPadCards = useMemo(() => {
    ...
    const result = [...ideationVideoCards, ...scratchpadLocalCards, ...filteredPending];

    // 🔴 디버깅: scratchPadCards 소스 구분
    console.log('[scratchPadCards] Card sources:', {
      ideationVideoCardIds: ideationVideoCards.map(c => c.id?.slice(0, 8)),
      scratchpadLocalCardIds: scratchpadLocalCards.map(c => c.id?.slice(0, 8)),
      filteredPendingIds: filteredPending.map(c => c.id?.slice(0, 8)),
  ...
  const setScratchPadCards = useCallback((updater: InsightCard[] | ((prev: InsightCard[]) => InsightCard[])) => {
    if (typeof updater === 'function') {
      ...
      const syncedIds = new Set(syncedCards.map(c => c.id));
      const persistedIds = new Set(persistedLocalCards.map(c => c.id));
      setPendingLocalCards(updater.filter(c => !syncedIds.has(c.id) && !persistedIds.has(c.id)));
    }
  ...
  useEffect(() => {
    ...
    console.log('[useEffect mandalaCards] Syncing to cards state:', {
      mandalaLocalCardsCount: mandalaLocalCards.length,
      mandalaVideoCardsCount: mandalaVideoCards.length,
      totalMandalaCards: allMandalaCards.length,
      mandalaCards: allMandalaCards.map(c => ({ id: c.id, cellIndex: c.cellIndex, levelId: c.levelId })),
    });
    ...
    if (allMandalaCards.length > 0) {
      setCards(prev => {
        ...
        const updatedPrev = prev.map(card => {
          ...
          if (mandalaCard) {
            ...
            if (cellIndexChanged || levelIdChanged) {
              hasChanges = true;
              console.log('[useEffect mandalaCards] Updating existing card position:', {
                id: card.id,
                oldCellIndex: card.cellIndex,
                newCellIndex: mandalaCard.cellIndex,
                oldLevelId: card.levelId,
                newLevelId: mandalaCard.levelId,
              });
        ...
        if (newCards.length > 0) {
          hasChanges = true;
          console.log('[useEffect mandalaCards] Adding new cards:', {
            existingCount: prev.length,
            newCardsCount: newCards.length,
            newCards: newCards.map(c => ({ id: c.id, cellIndex: c.cellIndex, levelId: c.levelId })),
          });
        }
  ...
  const cardsByCell = cards.reduce((acc, card) => {
  ...
  const handleCellClick = (cellIndex: number, subject: string) => {
  ...
  const handleNavigateToSubLevel = useCallback((subject: string, gridIndex: number) => {
  ...
  const handleSubjectsReorder = useCallback(async (newSubjects: string[], swappedIndices?: {from: number, to: number}) => {
    setMandalaLevels(prev => ({
      ...prev,
      [currentLevelId]: {
        ...prev[currentLevelId],
    ...
    if (swappedIndices) {
      ...
      for (const card of affectedCards) {
        const newCellIndex = card.cellIndex === swappedIndices.from
          ? swappedIndices.to
          : swappedIndices.from;
  ...
  }, [currentLevelId, toast, selectedCellIndex, cards, syncedCards, persistedLocalCards, updateVideoState, updateLocalCard]);
  ...
  const handleFileUpload = useCallback(async (file: File, cellIndex: number, levelId: string): Promise<InsightCard | null> => {
  ...
  const handleFileDrop = useCallback(async (cellIndex: number, files: FileList) => {
  ...
  const handleScratchPadFileDrop = useCallback(async (files: FileList) => {
    for (const file of Array.from(files)) {
  ...
  const handleCardDrop = useCallback(async (cellIndex: number, url?: string, cardId?: string, multiCardIds?: string[], files?: FileList) => {
    setIsDraggingOver(false);
    ...
    console.log('[handleCardDrop] 📍 Card drop initiated:', { cellIndex, url, cardId, multiCardIds });

    // Handle multi-card drop (process multiple cards at once)
    if (multiCardIds && multiCardIds.length > 0) {
      console.log('[handleCardDrop] 🔢 Processing multi-card drop:', multiCardIds.length);
      ...
      for (const id of multiCardIds) {
        ...
        if (!card) {
          console.warn('[handleCardDrop] ⚠️ Card not found:', id);
          continue;
        }

        console.log('[handleCardDrop] 📦 Processing card:', { id: id.slice(0, 8), source });

        try {
          if (source === 'synced') {
            // YouTube card: Update video_states table (MOVE, not COPY)
            await updateVideoState.mutateAsync({
              videoStateId: id,
              updates: {
                is_in_ideation: false,  // Remove from ideation
                cell_index: cellIndex,
                level_id: currentLevelId,
              }
            });
            console.log('[handleCardDrop] ✅ Synced card moved:', id.slice(0, 8));
          } else if (source === 'local') {
            // Local card: Update local_cards table
            await updateLocalCard({
              id,
              cell_index: cellIndex,
              level_id: currentLevelId,
            });
            console.log('[handleCardDrop] ✅ Local card moved:', id.slice(0, 8));
          } else if (source === 'pending') {
            // Pending card: Persist to local_cards first
            if (!canAddCard) {
              toast({
                title: "저장 한도 초과",
            ...
            console.log('[handleCardDrop] ✅ Pending card persisted:', id.slice(0, 8));

            // Remove from pending
            setPendingLocalCards(prev => prev.filter(c => c.id !== id));
          }
        } catch (error) {
          console.error('[handleCardDrop] ❌ Failed to move card:', id, error);
          toast({
            title: "이동 실패",
    ...
    if (cardId) {
      ...
      if (!card) {
        console.warn('[handleCardDrop] ⚠️ Single card not found:', cardId);
        return;
      }

      console.log('[handleCardDrop] 📦 Processing single card:', { id: cardId.slice(0, 8), source });
      ...
      try {
        if (source === 'synced') {
          ...
          console.log('[handleCardDrop] ✅ Synced card moved:', cardId.slice(0, 8));
        } else if (source === 'local') {
          // Local card: Update local_cards table
          await updateLocalCard({
            id: cardId,
          ...
          console.log('[handleCardDrop] ✅ Local card moved:', cardId.slice(0, 8));
        } else if (source === 'pending') {
          // Pending card: Persist to local_cards first
          if (!canAddCard) {
            toast({
          ...
          console.log('[handleCardDrop] ✅ Pending card persisted:', cardId.slice(0, 8));
        ...
        setCards(prev => {
          ...
          if (existing) {
            ...
            return prev.map(c => c.id === cardId ? { ...c, cellIndex, levelId: currentLevelId } : c);
          } else {
            // Add card to mandala
        ...
        console.error('[handleCardDrop] ❌ Failed to move card:', cardId, error);
        toast({
          title: "이동 실패",
          description: `카드 이동에 실패했습니다: ${error instanceof Error ? error.message : 'Unknown error'}`,
          variant: "destructive",
    ...
    if (url) {
      ...
      if (isValidUrl(url)) {
        ...
        fetchLinkTitle(url, linkType).then(title => {
          setCards(prev => prev.map(card =>
            card.id === newCard.id ? { ...card, title } : card
          ));
        });
  ...
  }, [currentLevel.subjects, scratchPadCards, syncedCards, persistedLocalCards, pendingLocalCards, toast, currentLevelId, handleFileDrop, isLoggedIn, navigate, updateLocalCard, updateVideoState, canAddCard, subscription, addLocalCard, setPendingLocalCards, setCards, setScratchPadCards]);

  const handleScratchPadDrop = useCallback((url: string) => {
  ...
  const handleScratchPadCardDrop = useCallback(async (cardId: string) => {
    // Moving card from mandala back to ideation
    const source = detectCardSource(cardId, syncedCards, persistedLocalCards);
    const card = cards.find(c => c.id === cardId);

    if (!card) {
      console.warn('[handleScratchPadCardDrop] ⚠️ Card not found:', cardId);
      return;
    }

    console.log('[handleScratchPadCardDrop] 🔙 Moving to ideation:', { id: cardId.slice(0, 8), source });

    try {
      if (source === 'synced') {
        // YouTube card: Update video_states table to move back to ideation
        await updateVideoState.mutateAsync({
          videoStateId: cardId,
          updates: {
            is_in_ideation: true,  // Add back to ideation
            cell_index: -1,
            level_id: 'scratchpad',
          }
        });
        console.log('[handleScratchPadCardDrop] ✅ Synced card moved to ideation:', cardId.slice(0, 8));
      } else if (source === 'local') {
        // Local card: Update local_cards table
        await updateLocalCard({
          id: cardId,
        ...
        console.log('[handleScratchPadCardDrop] ✅ Local card moved to ideation:', cardId.slice(0, 8));
      }

      // UI update: 만다라에서 카드 제거 (scratchPadCards는 refetch로 자동 업데이트)
      ...
      console.error('[handleScratchPadCardDrop] ❌ Failed to move card:', cardId, error);
      toast({
        title: "이동 실패",
  ...
  const handleScratchPadMultiCardDrop = useCallback(async (cardIds: string[]) => {
    // Moving multiple cards from mandala back to ideation
    console.log('[handleScratchPadMultiCardDrop] 🔙 Moving multiple cards to ideation:', cardIds.length);
    ...
    for (const cardId of cardIds) {
      ...
      if (!card) {
        console.warn('[handleScratchPadMultiCardDrop] ⚠️ Card not found:', cardId);
        continue;
      }

      console.log('[handleScratchPadMultiCardDrop] 📦 Processing card:', { id: cardId.slice(0, 8), source });
      ...
      try {
        if (source === 'synced') {
          ...
          console.log('[handleScratchPadMultiCardDrop] ✅ Synced card moved to ideation:', cardId.slice(0, 8));
        } else if (source === 'local') {
          // Local card: Update local_cards table
          await updateLocalCard({
            id: cardId,
          ...
          console.log('[handleScratchPadMultiCardDrop] ✅ Local card moved to ideation:', cardId.slice(0, 8));
        }
      } catch (error) {
        console.error('[handleScratchPadMultiCardDrop] ❌ Failed to move card:', cardId, error);
        toast({
          title: "이동 실패",
  ...
  const handleCardDragStart = (card: InsightCard) => {
    ...
    try {
      switch (source) {
        case 'synced':
          ...
          await updateVideoState.mutateAsync(
            { videoStateId: id, updates: { user_note: note } }
          );
          toast({
            title: "메모 저장됨",
      ...
      setCards(prev => prev.map(card =>
        card.id === id ? { ...card, userNote: note } : card
      ));
  ...
  }, [syncedCards, persistedLocalCards, scratchPadCards, cards, updateVideoState, updateLocalCard, addLocalCard, toast]);

  // Save watch position for YouTube videos (only for logged-in users with synced videos)
  const handleSaveWatchPosition = useCallback((id: string, positionSeconds: number) => {
  ...
  const handleCardsReorder = useCallback((reorderedCards: InsightCard[]) => {
    setCards(prev => {
  ...
  const handleDeleteCards = useCallback((cardIds: string[]) => {
    ...
    if (syncedToDelete.length > 0) {
      Promise.all(
        ...
        toast({
          title: "삭제 실패",
          description: error instanceof Error ? error.message : "일부 동영상을 삭제하지 못했습니다.",
          variant: "destructive",
        });
      });
    }
    ...
    if (persistedToDelete.length > 0) {
      Promise.all(
        persistedToDelete.map(id => deleteLocalCard(id))
      ).then(() => {
        toast({
          title: "삭제됨",
        ...
        toast({
          title: "삭제 실패",
          description: error instanceof Error ? error.message : "일부 카드를 삭제하지 못했습니다.",
          variant: "destructive",
        });
      });
    }
  ...
  const displayCards = selectedCellIndex !== null
    ? (cardsByCell[selectedCellIndex] || [])
    : currentLevelCards;

  const displayTitle = selectedCellIndex !== null
  ...
  const currentModalCard = useMemo(() => {
    ...
    const foundCard = cards.find(c => c.id === selectedCard.id)
    ...
    <div className="h-screen flex flex-col bg-surface-base overflow-hidden">
      ...
      {(isMandalaFloating || isMandalaFloatingMode) && (
        <FloatingMandala
          ...
          totalCards={Object.values(cardsByCell).reduce((sum, cards) => sum + cards.length, 0)}
