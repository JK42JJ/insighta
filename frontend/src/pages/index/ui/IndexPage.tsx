...
function AuthenticatedApp() {
  ...
  // -- Shell store sync (AppShell reads these) --
  const { setMinimapData, setSearchBarElement, setOnNavigateHome, clearShell } = useShellStore();

  const navigateHome = useCallback(() => navigation.handleNavigate('root'), [navigation]);

  useEffect(() => {
    setOnNavigateHome(navigateHome);
    return () => clearShell();
  }, [navigateHome, setOnNavigateHome, clearShell]);

  const minimapDataMemo = useMemo(
    () => ({
      cardsByCell: cards.cardsByCell,
  ...
  useEffect(() => {
    setMinimapData(minimapDataMemo);
  }, [minimapDataMemo, setMinimapData]);
