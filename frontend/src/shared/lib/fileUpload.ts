...
export const uploadFile = async (file: File): Promise<{ url: string; fileName: string } | null> => {
  try {
    ...
    const { error } = await supabase.storage.from('insight-files').upload(filePath, file);
    ...
    // Get public URL
    const { data: urlData } = supabase.storage.from('insight-files').getPublicUrl(filePath);
