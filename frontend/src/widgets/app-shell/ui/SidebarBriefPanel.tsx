            ...
            return (
              <li key={it.slug} className="mb-0.5">
                <button
                  ...
                  className={cn(
                    'flex w-full flex-col gap-0.5 pl-3.5 pr-2 py-1.5 text-left leading-[1.5] transition-colors',
                    current
                ...
                >
                  {/* Line 1 identifies the issue: number and date. The headline is
                      a 40-character sentence, so it goes on its own two lines
                      below rather than being cut to an ellipsis beside the label. */}
                  <span className="flex w-full items-center gap-2">
                    <span className="shrink-0 tabular-nums">{it.issueLabel}</span>
                    <span className="min-w-0 flex-1 truncate text-[12px] text-sidebar-foreground/45">
                      {it.dateLabel}
                    </span>
                    {!it.read && (
                      <span
                  ...
                  <span className="line-clamp-2 text-[12.5px] leading-[1.45] text-sidebar-foreground/60">
                    {it.headline}
                  </span>
                </button>
