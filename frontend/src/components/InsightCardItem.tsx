...
export function InsightCardItem({ card, onClick, onCtrlClick, onDragStart, onInternalDragStart, onSave, isDraggable = true }: InsightCardItemProps) {
  ...
  const createTimestampLink = (timestamp: string, totalSeconds: number, label: string, videoUrl: string): string => {
  ...
  const processDroppedData = (e: DragEvent<HTMLTextAreaElement | HTMLDivElement>) => {
    ...
    if (droppedUrl && (droppedUrl.includes("youtube.com") || droppedUrl.includes("youtu.be"))) {
      ...
      if (urlTimestamp) {
        const link = createTimestampLink(urlTimestamp.timestamp, urlTimestamp.totalSeconds, "", card.videoUrl);
    ...
    if (droppedText) {
      ...
      if (extracted) {
        ...
        const link = createTimestampLink(extracted.timestamp, totalSeconds, extracted.label, card.videoUrl);
        const cursorPos = textareaRef.current?.selectionStart || note.length;
        const newNote = note.slice(0, cursorPos) + (note.length > 0 && cursorPos > 0 ? "\n" : "") + link + note.slice(cursorPos);
        setNote(newNote);
        toast.success("타임스탬프 링크가 추가되었습니다");
  ...
  const handleDragStart = (e: React.DragEvent) => {
    ...
    try {
      ...
      console.log('[InsightCardItem] dragStart', { id: card.id, isFlipped, isDraggable });
    } catch {}
          ...
          <div className="p-3 h-full flex flex-col relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-3 flex-shrink-0">
              <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <span className="text-xs font-semibold text-foreground whitespace-nowrap">메모 편집</span>
              </div>
