...
export function MandalaDashboard({
  ...
  const cardsWithMemo = Object.values(cardsByCell).flat().filter(c => c.userNote && c.userNote.trim().length > 0).length;
  ...
  const pages = [
    {
      title: "Focus Map",
      subtitle: "어디에 (Where)",
      description: "나의 노력과 관심사 분포",
    },
    {
      title: "월별 인사이트 축적 트렌드",
      subtitle: "얼마나 (How Fast)",
      description: "지식 자산의 성장 속도와 꾸준함",
    },
    {
      title: "저장된 인사이트 품질 분석",
      subtitle: "어떻게 (How Well)",
      description: "학습의 질과 참여 수준",
    },
  ];
        ...
        >
          <div className="h-full flex flex-col">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-muted-foreground">R2 6개월 B2. 64 insights/web</span>
            </div>
        ...
        >
          <div className="h-full flex flex-col">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-muted-foreground">R2 6개월 B2. 64 insights/month</span>
            </div>
            ...
            <div className="flex-1 flex gap-4">
              ...
              <div className="flex-1 flex items-center justify-center">
                <div className="relative w-32 h-32">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-[8px] text-muted-foreground">총 카드</span>
                  </div>
                </div>
              </div>
              ...
              <div className="w-28 flex flex-col justify-center gap-2 text-[9px]">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="font-medium text-foreground">메모 있음</div>
                    <div className="text-muted-foreground">{cardsWithMemo}개 ({totalCards > 0 ? Math.round((cardsWithMemo / totalCards) * 100) : 0}%)</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-sm bg-primary/40" />
                  <div>
                    <div className="font-medium text-foreground">메모 없음</div>
                    <div className="text-muted-foreground">{cardsWithoutMemo}개 ({totalCards > 0 ? Math.round((cardsWithoutMemo / totalCards) * 100) : 0}%)</div>
                  </div>
                </div>
              </div>
            </div>
