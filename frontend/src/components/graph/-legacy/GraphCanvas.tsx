// @deprecated — replaced by the Sigma.js implementation in ../GraphCanvas.tsx
// (feat/graph-sigma). Kept per the no-component-deletion rule for instant
// rollback: swap the import in GraphView.tsx back to this file. Not imported
// anywhere; tree-shaken out of the bundle.
// ============================================================================
...
export function GraphCanvas({
  ...
  const linkColor = useCallback((link: { source: unknown; target: unknown; relation: string }) => {
    const s = stateRef.current;
    ...
    if (s.hasHighlight && !isConnected)
      return s.isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)';
    if (s.hasMandalaHighlight && !isMandalaEdge)
      return s.isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)';
