...
import { Sparkles, Minimize2, Maximize2, GripHorizontal, GripVertical, X, Move } from "lucide-react";
...
export const FloatingMandala = forwardRef<HTMLDivElement, FloatingMandalaProps>(function FloatingMandala({
  centerGoal,
  totalCards,
  isMinimized,
  onToggleMinimized,
  ...
  initialPosition,
  onPositionChange,
}: FloatingMandalaProps, forwardedRef) {
  ...
  const handleDockedDragMouseDown = useCallback((e: React.MouseEvent) => {
  ...
  }, [isDragging, pendingDock, detectDockPosition, onDockPositionChange, onToggleFloating, position.x, position.y, onPositionChange]);
  ...
  const DockZoneIndicators = () => createPortal(
    <div 
