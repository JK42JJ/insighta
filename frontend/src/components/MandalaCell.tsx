...
export interface MandalaCellProps {
  ...
  onDrop: (index: number, url?: string, cardId?: string, multiCardIds?: string[], files?: FileList) => void;
...
export function MandalaCell({
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    try {
      ...
      const debugUrl = e.dataTransfer.getData("text/uri-list") || e.dataTransfer.getData("text/plain");
      // eslint-disable-next-line no-console
      console.log("[MandalaCell] drop", { index, types, debugCardId, debugMulti, debugUrl, hasFiles: e.dataTransfer.files.length > 0 });
    } catch {}
  ...
  return (
    <div
      className={cn(
        ...
        // Default state
        !isCenter && !isSelected && "border-border/40 hover:border-primary/40 hover:-translate-y-0.5",
        // Center cell
        ...
        // Cell swap target
        isCellSwapTarget && !isCenter && "ring-2 ring-accent-foreground ring-offset-2 ring-offset-background scale-105"
      )}
      style={{ 
        boxShadow: isSelected || isDropTarget ? 'var(--shadow-lg)' : isCenter ? 'var(--shadow-inset-raised)' : 'var(--shadow-sm)'
      }}
    ...
    >
      ...
      <span className={cn(
        ...
        isCenter ? "text-primary text-sm md:text-base font-semibold" : "text-foreground/90 text-xs md:text-sm",
