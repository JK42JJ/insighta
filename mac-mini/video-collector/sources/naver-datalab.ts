 ...
 * orchestrator will then use Google Trends fallback.
 */
export async function collectNaverDataLab(
  ...
  try {
    ...
    return { keywords: [], diagnostics: { error: (e as Error).message } };
  }
  ...
  if (!res.ok) {
    ...
    return {
      keywords: [],
      diagnostics: { status: res.status, error: text.slice(0, 200) },
    };
  }

  const json = (await res.json()) as { results?: DataLabResultRow[] };
  const groups = json.results ?? [];
  // Score each group by the last 7 days' average ratio.
  ...
  // Return the top groups; their *seed keywords* are the search terms.
  const topGroups = scored.slice(0, opts.topN).map((g) => g.title);
