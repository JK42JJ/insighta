...
export const FloatingScratchPad = forwardRef<HTMLDivElement, FloatingScratchPadProps>(
  function FloatingScratchPad(
    ...
    const DockZoneIndicators = forwardRef<HTMLDivElement>(function DockZoneIndicators(_props, ref) {
      ...
      const indicators = (
        <div ref={ref} className="dock-zone-indicators" style={{ position: 'fixed', inset: 0, zIndex: 99999, pointerEvents: 'none' }}>
          <div className={cn('absolute left-0 right-0 transition-all duration-300 ease-out', pendingDock === 'top' ? 'opacity-100' : 'opacity-0')} style={{ top: `${HEADER_HEIGHT}px`, height: '2px' }}>
            <div className="h-full w-full" style={{ background: 'hsl(var(--primary) / 0.35)', boxShadow: '0 1px 6px 0 hsl(var(--primary) / 0.15)' }} />
          </div>
          <div className={cn('absolute bottom-0 left-0 right-0 transition-all duration-300 ease-out', pendingDock === 'bottom' ? 'opacity-100' : 'opacity-0')} style={{ height: '2px' }}>
            <div className="h-full w-full" style={{ background: 'hsl(var(--primary) / 0.35)', boxShadow: '0 -1px 6px 0 hsl(var(--primary) / 0.15)' }} />
          </div>
          <div className={cn('absolute left-0 transition-all duration-300 ease-out', pendingDock === 'left' ? 'opacity-100' : 'opacity-0')} style={{ top: `${HEADER_HEIGHT}px`, bottom: 0, width: '2px' }}>
            <div className="h-full w-full" style={{ background: 'hsl(var(--primary) / 0.35)', boxShadow: '1px 0 6px 0 hsl(var(--primary) / 0.15)' }} />
          </div>
          <div className={cn('absolute right-0 transition-all duration-300 ease-out', pendingDock === 'right' ? 'opacity-100' : 'opacity-0')} style={{ top: `${HEADER_HEIGHT}px`, bottom: 0, width: '2px' }}>
            <div className="h-full w-full" style={{ background: 'hsl(var(--primary) / 0.35)', boxShadow: '-1px 0 6px 0 hsl(var(--primary) / 0.15)' }} />
          </div>
        </div>
      );
      ...
      return (
        ...
        >
          <div className={cn('relative overflow-hidden bg-muted rounded-sm', cardSize)} style={{ boxShadow: 'var(--shadow-sm)' }}>
            ...
            {isSelected && (
              <div className="absolute top-0.5 left-0.5 bg-primary rounded-full p-0.5 cursor-pointer" onClick={(e) => { e.stopPropagation(); setSelectedCardIds((prev) => { const next = new Set(prev); next.delete(card.id); return next; }); }}>
                <Check className={checkSize} style={{ color: 'hsl(var(--primary-foreground))' }} />
              </div>
            )}
            {!isCompact && (
              <a href={card.videoUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="absolute top-0.5 right-0.5 z-10 bg-background/90 p-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity text-primary">
          ...
          <div className={cn('flex w-full transition-all duration-300 justify-center', isAnimating && 'animate-fade-in')}>
            ...
            >
              ...
              <div className="flex items-center gap-3 px-3 py-1.5">
                <div className="flex items-center gap-2 flex-shrink-0 cursor-grab active:cursor-grabbing select-none" onMouseDown={handleDockedDragMouseDown}>
                  <GripHorizontal className="w-3 h-3 text-muted-foreground/40" />
                  <div className="flex items-center gap-1.5">
                    <Lightbulb className="w-3.5 h-3.5 text-primary/80" />
                    <span className="text-xs font-medium text-foreground/80">{t('ideation.title')}</span>
                    {cards.length > 0 && <span className="text-[10px] text-primary/70 font-medium bg-primary/10 px-1.5 py-0.5 rounded-full">{cards.length}</span>}
                  </div>
                </div>
                <div className="h-4 w-px bg-border/40 flex-shrink-0" />
                <button onClick={() => scrollByAmount('left')} className={cn('flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center transition-all duration-200', 'bg-surface-base/80 hover:bg-surface-base border border-border/50', 'text-muted-foreground hover:text-foreground', canScrollLeft ? 'opacity-100' : 'opacity-0 pointer-events-none')} style={{ boxShadow: 'var(--shadow-sm)' }}>
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <div ref={scrollContainerRef} className="flex-1 flex items-center gap-1.5 overflow-x-auto scrollbar-none py-0.5 scroll-smooth" onClick={handleContainerClick}>
                  {cards.length === 0 ? (
                ...
                <button onClick={() => scrollByAmount('right')} className={cn('flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center transition-all duration-200', 'bg-surface-base/80 hover:bg-surface-base border border-border/50', 'text-muted-foreground hover:text-foreground', canScrollRight ? 'opacity-100' : 'opacity-0 pointer-events-none')} style={{ boxShadow: 'var(--shadow-sm)' }}>
                  <ChevronRight className="w-4 h-4" />
                </button>
                <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground/60 hover:text-foreground hover:bg-muted/50 flex-shrink-0" onClick={onToggleFloating} title={t('mandala.switchToFloating')}>
        ...
        <>
          ...
          >
            ...
            <div className="flex flex-col h-full py-1.5 px-1.5 gap-1.5">
              <div className="flex items-center justify-center gap-1 flex-shrink-0 cursor-grab active:cursor-grabbing select-none py-1" onMouseDown={handleDockedDragMouseDown}>
                <GripVertical className="w-3 h-3 text-muted-foreground/50" />
                <div className="p-0.5 rounded bg-primary/10">
                  <Lightbulb className="w-2.5 h-2.5 text-primary" />
                </div>
                {cards.length > 0 && <span className="text-[9px] text-primary font-medium">{cards.length}</span>}
                <Button variant="ghost" size="icon" className="h-4 w-4 text-muted-foreground hover:text-foreground ml-auto" onClick={onToggleFloating} title={t('mandala.switchToFloating')}>
                  <Move className="w-2.5 h-2.5" />
                </Button>
              </div>
              <button onClick={() => scrollVerticalByAmount('up')} className={cn('flex-shrink-0 w-full h-6 rounded flex items-center justify-center transition-all duration-200', 'bg-surface-base/80 hover:bg-surface-base border border-border/50', 'text-muted-foreground hover:text-foreground', canScrollUp ? 'opacity-100' : 'opacity-0 pointer-events-none')} style={{ boxShadow: 'var(--shadow-sm)' }}>
                <ChevronUp className="w-3.5 h-3.5" />
              </button>
              <div ref={verticalScrollRef} className="flex-1 flex flex-col gap-1 overflow-y-auto scrollbar-none scroll-smooth" onClick={handleContainerClick}>
                {cards.length === 0 ? (
                  <div className="flex flex-col items-center justify-center gap-1 text-muted-foreground py-3 flex-1">
                    <Plus className="w-3 h-3 opacity-50" />
                    <span className="text-[9px] text-center leading-tight whitespace-pre-line">{t('ideation.emptyHintVertical')}</span>
                  </div>
                ) : (
              ...
              <button onClick={() => scrollVerticalByAmount('down')} className={cn('flex-shrink-0 w-full h-6 rounded flex items-center justify-center transition-all duration-200', 'bg-surface-base/80 hover:bg-surface-base border border-border/50', 'text-muted-foreground hover:text-foreground', canScrollDown ? 'opacity-100' : 'opacity-0 pointer-events-none')} style={{ boxShadow: 'var(--shadow-sm)' }}>
    ...
    return (
      <>
        ...
        <div
          ...
          onDrop={handleDrop}
        >
          <div className={cn('flex items-center justify-between px-3 py-2 border-b border-border/30 cursor-grab select-none', isDragging && 'cursor-grabbing')} onMouseDown={handleDragMouseDown}>
            <div className="flex items-center gap-2">
              ...
              {cards.length > 0 && <span className="text-[10px] text-primary font-medium bg-primary/10 px-1.5 py-0.5 rounded">{t('common.items', { count: cards.length })}</span>}
              {selectedCardIds.size > 0 && (
                <>
                  <span className="text-[10px] bg-primary text-primary-foreground px-1.5 py-0.5 rounded-full">{t('common.selected', { count: selectedCardIds.size })}</span>
                  <button onClick={(e) => { e.stopPropagation(); onDeleteCards?.([...selectedCardIds]); setSelectedCardIds(new Set()); setLastSelectedIndex(null); }} onMouseDown={(e) => e.stopPropagation()} className="p-1 rounded-lg text-destructive hover:bg-destructive/10 transition-colors" title={t('cards.deleteSelected')}>
            ...
            <div className="flex items-center gap-0.5" onMouseDown={(e) => e.stopPropagation()}>
              ...
              <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-foreground" onClick={onToggleFloating} title={t('mandala.switchToDock')}>
          ...
          {!isMinimized && (
            <div className="p-3 overflow-hidden" style={{ height: 'calc(100% - 44px)' }}>
              ...
              {cards.length === 0 ? (
                ...
                <div ref={floatingContentRef} className="flex flex-wrap gap-2 overflow-y-auto h-full content-start p-1 relative" onClick={handleContainerClick}>
          ...
          {!isMinimized && (
            <div className="absolute bottom-0 right-0 w-5 h-5 cursor-se-resize flex items-center justify-center" onMouseDown={handleResizeMouseDown}>
