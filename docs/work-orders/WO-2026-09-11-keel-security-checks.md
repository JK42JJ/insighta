...
- [ ] keel.yml 예약 실행에서 `supply-chain` 이 npm audit 수치를 남김 — #1638 머지 후 다음 30분 주기
...
- 예약 실행 확인: 2026-09-11 20:47 KST(run 34595765253) 에서 `iam-hygiene` 이 OIDC 역할로 credential report 를 읽어 ok:false 를 남김. `supply-chain` 은 그 시점 main 이 Dependabot API 판이라 "alerts unavailable" — #1638 머지 후 다음 주기에 재확인. 나머지 3종(cloud-posture · k8s-hardening · secret-exposure)은 S3.
