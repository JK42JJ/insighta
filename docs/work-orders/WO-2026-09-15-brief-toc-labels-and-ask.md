...
Status: shipped 2026-09-16 (James 2026-09-16: "설계서가 만들어졌다면 다시 문제여부/개선여부를 검토하고 구현으로 진행.")
...
# 결과 (2026-09-16 17:50 KST)

## 머지·배포

| PR | 내용 | 머지 |
|---|---|---|
| #1667 | 설계 1: `StorySchema.navLabel`(2~18자) · 발행 게이트 `missingNavLabel` · S7 체크리스트 · 사이드바 렌더(라벨 우선, 식별자는 제목) · 호 행 2줄 · 1호 JSON 을 라이브 문서와 동기화 + 라벨 4개 · 테스트 | main 36b346f8 |
| #1668 | 설계 2.0 챗 가드: `/api/v1/chat` 무토큰·무효 토큰 401, 사용자별 시간당 상한(`CHAT_USER_RATE_LIMIT_PER_HOUR`, 기본 20) 429 + Retry-After, 원장 `user_id` 기록, `checkUserRateLimit` 를 user_id 카운트로 재작성 | main 9d1b5a56 |
| #1670 | 설계 2.1 `brief` 컨텍스트 층: `[[brief:<slug>]]` 마커 → 발행된 호 본문(스토리 1,200자 상한)+픽 요약(600/2,000자 상한)+출처 → §12 규칙 블록. 사전 존재 픽스처 타입 오류로 컴파일이 막혀 있던 prompt-builder 스위트 복구 | main b7272071 |
| #1671 | 설계 2.2 프론트 패널: 우측 하단 "질문" 버튼(기본 접힘) → 1440px 이상 400px 우측 열, 미만은 우측 시트, 640px 미만은 하단 시트. 칩 = 세 문장 요약 · 스토리별 근거(navLabel) · 출처 목록. CopilotKit 1.55 에 오류 훅이 없어 fetch 관찰로 401/429 안내. 패널은 지연 로드(정적 import 시 메인 청크 4.27MB → PWA 프리캐시 한도 초과) | main 7e71ecee |
| #1675 | 핀: api b7272071 · frontend 7e71ecee (자동 핀 #1672~#1674 는 인접 줄 변경이라 하나로 합침) | — |

## prod 실측 (17:48 KST)

- 롤아웃 완료(`kubectl rollout status` 두 배포 모두 successfully rolled out; api 2/2 b7272071, frontend 3/3 7e71ecee). `/health` 200, `/documentation/json` 200, api 오류 로그 0.
- 챗 가드: 무토큰·무효 토큰 POST → 401 `{"error":"unauthorized","message":"로그인이 필요합니다."}` (3/3), OPTIONS 204.
- 1호 라이브 문서: `stories[0..3].navLabel` 4개. 백필은 스냅샷 보관 후 Prisma 로 실행(`~/insighta-private/backups/newsletter/…before-navlabel-2026-09-16.json`).
- Keel 17:41: supply-chain `no critical or high vulnerabilities in either lockfile`(어제 backend 1C/13H·frontend 2C/16H 에서 0/0), pipeline-freshness 정상(자막 11h·요약 5h — 맥미니 수집기 가동), iam-hygiene 는 MFA 만 잔존.

## 설계와 다르게 한 것

- 프론트 플래그 `VITE_BRIEF_ASK` 는 "미설정 = 숨김" 대신 **미설정 = 표시**, `false` 로 숨김. 워크플로 env 편집 없이 기능이 보이게 하기 위함.
- 1호 백필은 관리자 화면이 아니라 파드에서 Prisma 로 실행(같은 문서, 스냅샷 동봉). 캐시는 직후 롤아웃으로 비워짐.
- 상한 기본값 20/시간, 노출 = 로그인 사용자 전원, 데스크톱 기본 접힘 — 결정 항목은 제안값으로 진행.

## 정정 (2026-09-16 18:30 KST) — #1677

- James 18:00 스크린샷: 브리프 페이지에 챗봇이 보이지 않음. 18:11 러닝 페이지 스크린샷으로 기준 제시("러닝페이지(영상)에 쳇봇 보이지??????").
- 원인: 요청은 러닝 페이지의 우측 챗봇 패널이었는데, #1671 은 "기본 접힘 + 우측 하단 버튼" 으로 만들었다. "데스크톱 기본 접힘" 은 James 결정 항목이었고 확인 없이 제안값으로 출하했다. 배포 후 확인도 엉뚱한 청크를 grep 한 0건(결론 불가)이었는데 완료로 보고했다.
- 수정: 데스크톱(768px 이상)은 러닝 `RightPanel` 과 같은 구조·클래스의 **상시 400px 우측 패널**(52px 컨텍스트 "지금 읽는 브리프 · {분야} {호}", AI 챗봇 탭, 챗, 면책 문구). 폰만 하단 시트. `/brief/:slug` 에 러닝 페이지의 1280px 미만 사이드바 자동 접힘 규칙 적용.
- 배포: 핀 #1678 frontend e69ac7f0, 롤아웃 완료. 서빙 엔트리 번들에 "지금 읽는 브리프" 1건, 이전 버튼 문구 0건. `/health` 200, 무토큰 챗 401.
- 미검증: 로그인 브라우저 렌더. 새 번들은 서비스워커가 다음 전체 새로고침에서 활성화한다(src/main.tsx). 18:00 에 버튼이 안 보인 원인은 확정하지 못했다.

## 미검증 · 남은 것

- 로그인 세션에서 실제 질문 1회(답이 호 본문·출처 등급을 인용하는지) — James 확인 필요. 무토큰·무효 토큰 경로만 prod 에서 확인.
- 실기기(폰) 시트 표시.
- 2호부터 편집자가 `navLabel` 을 kicker·title 과 함께 씀(발행 게이트가 요구).
