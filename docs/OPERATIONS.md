...
[EC2 t2.micro (Ubuntu 22.04, us-west-2)]
    ...
    │
    ├── Docker: tubearchive-api (Fastify, port 3000)
    │     ├── Prisma ORM → PostgreSQL
    │     ├── YouTube Data API v3
    │     ├── Gemini AI API
    │     └── JWT 검증 (Supabase)
    │
    └── Docker: tubearchive-frontend (Nginx, port 8081)
          └── React SPA (Vite 빌드)
...
Internet → :443 (Nginx SSL) → :3000 (API) / :8081 (Frontend)
                              ...
                              Docker bridge network (tubearchive-network)
```
...
| SSH Key | `~/Downloads/prx01-tubearchive.pem` |
| 앱 디렉토리 | `/opt/tubearchive/` |
...
|-----------|--------|------|-------------|----------|
| `tubearchive-api` | `ghcr.io/jk42jj/insighta-api:latest` | 127.0.0.1:3000 | 512MB | `curl http://localhost:3000/health` (30s 간격) |
| `tubearchive-frontend` | `ghcr.io/jk42jj/insighta-frontend:latest` | 127.0.0.1:8081 | 256MB | `wget http://localhost:8081/` (30s 간격) |

### Docker Compose 파일 위치

- **EC2**: `/opt/tubearchive/docker-compose.prod.yml`
...
# 컨테이너 로그 (실시간)
docker logs tubearchive-api --tail 50 -f
docker logs tubearchive-frontend --tail 50 -f
...
```bash
# EC2에서 실행
docker inspect --format='{{.State.Health.Status}}' tubearchive-api       # healthy
...
사이트 접속 불가
    ...
    │
    ├── Docker 확인: docker ps
    │     ├── 컨테이너 없음: cd /opt/tubearchive && docker compose up -d
...
```bash
# 1. API 로그 확인
docker logs tubearchive-api --tail 100

# 2. DB 연결 확인
docker exec tubearchive-api node -e "
...
4. Save rules
5. 접속 테스트: `ssh -i ~/Downloads/prx01-tubearchive.pem ubuntu@44.231.152.49`
...
| 위치 | 용도 | 접근 방법 |
|------|------|-----------|
| `/opt/tubearchive/.env` (EC2) | 프로덕션 런타임 | SSH 접속 |
| `.env` (로컬) | 개발 환경 | 직접 편집 |
...
| 로그 | 위치 | 크기 제한 |
|------|------|-----------|
| API 컨테이너 | `docker logs tubearchive-api` | JSON, 10MB × 3파일 |
| Frontend 컨테이너 | `docker logs tubearchive-frontend` | JSON, 10MB × 3파일 |
...
```bash
# 최근 100줄 + 실시간 추적
docker logs tubearchive-api --tail 100 -f

# 특정 시간 이후 로그
docker logs tubearchive-api --since "2026-03-04T00:00:00"

# 에러만 필터
docker logs tubearchive-api 2>&1 | grep -i error
...
```bash
# EC2에서 실행
docker run --rm -v tubearchive_cache_data:/data -v /tmp:/backup alpine tar czf /backup/cache_backup.tar.gz /data
docker run --rm -v tubearchive_logs_data:/data -v /tmp:/backup alpine tar czf /backup/logs_backup.tar.gz /data
```
...
```bash
docker inspect --format='{{json .State.Health}}' tubearchive-api | python3 -m json.tool
```
...
| `deploy/nginx/tubearchive.conf` | Nginx 설정 |
