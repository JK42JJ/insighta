...
export function registerEnrichCommands(program: Command): void {
  program
    ...
    .action(
      async (opts: { userId: string; limit: string; delay: string; skipBackfill?: boolean }) => {
        ...
        try {
          ...
          if (!opts.skipBackfill) {
            ...
            console.log(
              `  \u2192 Created: ${backfillResult.created} / ${backfillResult.total} (skipped: ${backfillResult.skipped}, errors: ${backfillResult.errors.length})`
            );
          }
