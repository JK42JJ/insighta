...
- analysis.mandala_fit.goal_contribution_pct: integer 0-100 — ask ONE question: "if a person whose goal is the MANDALA CENTER GOAL below watches this video, do they make REAL PROGRESS toward that goal?" Score the progress, NOT topical overlap.
  ...
  * CELL-MEDIATED CONTRIBUTION COUNTS: the CELL GOAL below is one concrete sub-goal OF the center goal — a video that gives real, usable means for the CELL GOAL is contributing to the center goal THROUGH that cell. Example: center goal "행복한 삶을 위한 나만의 프레임워크 만들기" + cell goal "스트레스 회복력 기르기" + video "수면 명상 음악 30분 — 만성피로 회복" → a usable recovery tool for that cell → 68. Contrast with the spot-the-difference example above: a brain-game is generic pastime content that serves NO cell of this framework specifically → stays ≤40.
  * INGREDIENTS COUNT for build/design goals: when the center goal is to BUILD or DESIGN something (a framework, plan, routine), videos supplying its ingredients — principles, criteria, domain overviews like "행복한 삶을 사는 10가지 방법" — are REAL PROGRESS (≥65), not surface overlap.
  * PASSIVE NEWS/REPORTAGE is NOT progress: a news segment merely describing a social trend is information consumption that leaves the viewer no closer to the goal (≤55).
  * DO NOT hedge in the 55-60 band. When uncertain, decide with this tie-breaker: "would a coach hired for THIS center goal put this video on the member's plan?" Plan-worthy → ≥65. Matches a cell's WORDING but a coach for the center goal would not assign it → ≤50. Example contrast for center "행복한 삶을 위한 나만의 프레임워크 만들기": "수면 명상 음악 — 만성피로 회복" serves the framework's recovery pillar → 68; "여드름 스킨케어 루틴" matches the 건강 cell's wording but no happiness-framework coach would assign it → 45.
