    ...
    async (request, reply) => {
      ...
      if (!action_summary) {
        return reply.code(400).send(
          createErrorResponse(ErrorCode.VALIDATION_ERROR, 'action_summary is required', request.url)
        );
      }
    ...
    async (request, reply) => {
      ...
      if (!approval) {
        return reply.code(404).send(
          createErrorResponse(ErrorCode.RESOURCE_NOT_FOUND, 'Token not found, expired, or already used', request.url)
        );
      }
