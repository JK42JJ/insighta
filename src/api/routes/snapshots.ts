...
export const snapshotRoutes: FastifyPluginCallback = (fastify, _opts, done) => {
  ...
  fastify.post('/', { onRequest: [fastify.authenticate] }, async (request, reply) => {
    ...
    if (!reason) {
      return reply
        .code(400)
        .send(createErrorResponse(ErrorCode.VALIDATION_ERROR, 'reason is required', request.url));
    }
   ...
   */
  fastify.post('/:id/rollback', { onRequest: [fastify.authenticate] }, async (request, reply) => {
    const userId = request.user.userId;
    ...
    if (!snapshot) {
      return reply
        .code(404)
        .send(createErrorResponse(ErrorCode.RESOURCE_NOT_FOUND, 'Snapshot not found', request.url));
    }
